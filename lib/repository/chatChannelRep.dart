...
class ChatChannelRepository {
  ...
  Future<DocumentReference> createchatChannel(ChatChannelModel chatChannelModel) async {
  ...
  Future<ChatChannelModel> getchatChannelById(String id) async {
    ...
    Future<ChatChannelModel> getchatChannelByuserList(List<String> userEmailList) async {
    ChatChannelModel chatChannel;
    final docSnapshot = await collection
  ...
  //     Future<List<ChatChannelModel>> getchatChannelByuserEmail(String userEmail) async {
  //   ChatChannelModel chatChannel;
  ...
  Future<void> addMessage(String chatChannelId, chatMessageData, partnerEmail) async{
...
await collection
    ...
    final docSnapshot =  collection
      ...
      snapshot.documents.forEach((element) {       
                element.data()['readingUserEmailList'].forEach((key,val)
          {
                if(key==partnerEmail)
                {
                  if(!val || !element.data()['onlineUserEmailList'].contains(partnerEmail))
                  {
                    var currentUnreadMsg=element.data()['unreadMessageNumberList'][key];
    ...
    Future<void> updateunreadMessageNumber(String chatChannelId, partnerEmail) async{
      ...
      snapshot.documents.forEach((element) {       
        ...
        if((element.data()['unreadMessageNumberList']==null))
        {
                ...
                element.data()['readingUserEmailList'].forEach((key,val)
          {
            ...
            {'unreadMessageNumberList': { 
              Constants.myEmail : 0,
              partnerEmail : element.data()['unreadMessageNumberList'][partnerEmail],
...
collection
        .where("userEmail", arrayContains: Constants.myEmail).get().then((var snapshot) {
      print(snapshot);
      snapshot.documents.forEach((element) {

        if((element.data()['onlineUserEmailList']==null || element.data()['onlineUserEmailList']?.length==0) && online)
        {
          collection.doc(element.id).update({'onlineUserEmailList':onlineEmail}).catchError(
          (error) {},
        );
        }
        else
        {
          ...
          collection.doc(element.id).update({'onlineUserEmailList':oflineEmail}).catchError(
          (error) {},
        );
        }
        }
      ...
      // List onlineEmail=["namra.patel.549@gmail.com"];
collection
        .where("channelID", isEqualTo: channelID).get().then((var snapshot) {
      print(snapshot);
      snapshot.documents.forEach((element) {
        ...
        if((element.data()['readingUserEmailList']==null) && reading)
        {
          collection.doc(element.id).update(
            {'readingUserEmailList': { 
              element.data()['userEmail'].first : element.data()['userEmail'].first==Constants.myEmail ? true :false,
              element.data()['userEmail'].last : element.data()['userEmail'].first==Constants.myEmail ? true :false,
        ...
        else
        {
          ...
          collection.doc(element.id).update({'readingUserEmailList': {
